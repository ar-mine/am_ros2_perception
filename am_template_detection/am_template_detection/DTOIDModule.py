...
class DTOIDModule:
    def __init__(self, template_dir, backend="cuda", no_filter_z=True):
        ...
        for t in tqdm(self.template_paths):
            ...
            elif iteration == (len(self.template_paths) - 1 - invalid_count):
    ...
    def process(self, rgb_img: np.ndarray, threshold: float = 0.7) -> (bool, np.ndarray, tuple, float):
        ...
        top_k = 1
        # Keep top 1 (eval)
        pred_scores_np = pred_scores_np[:top_k]
        pred_bbox_np = pred_bbox_np[:top_k]
        pred_template_ids = pred_template_ids[:top_k]
        if not self.no_filter_z:
            pred_z = pred_z[:top_k]
        ...
        for i in range(top_k):
            if i > pred_bbox_np.shape[0]-1:
                continue
            pred_bbox_np_i = pred_bbox_np[i]
            pred_scores_np_i = pred_scores_np[i]
            if len(pred_bbox_np_i) > 0 and pred_scores_np_i >= threshold:
                x1, y1, x2, y2 = pred_bbox_np_i
                score = pred_scores_np_i
        ...
        return success, img_copy, bbox, angel, score
